...
public class GamePanel extends JPanel implements KeyListener {
    ...
    public GamePanel() {   // 构造方法
        // 主图片采用宽800高300的彩色图片--图片不透明
        image = new BufferedImage(800, 300, BufferedImage.TYPE_INT_BGR);
        g2 = image.createGraphics();// 获取主图片绘图对象--使主图片image成为绘图对象
    ...
    private void paintImage() {
        ...
        // 绘制分数--drawString(String str, int x, int y)
        g2.drawString(String.format("%06d", score), 700, 30);// 绘制分数

        addObstacleTimer += FREASH;// 障碍计时器递增--20毫秒频率刷新一次
    ...
    public void gameOver() {
        ScoreRecorder.addNewScore(score);// 记录当前分数--添加新分数
        finish = true;
    }
